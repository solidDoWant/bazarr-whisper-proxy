...
async def test_whitespace_only_transcript_raises():
    audio = _load_fixture()
    with pytest.raises(AlignmentFailed):
        await align(audio, "   \t\n  ", FIXTURE_LANGUAGE, _settings=_default_settings())
...
async def test_silence_audio_no_hang():
    ...
    try:
        result = await asyncio.wait_for(
            align(silence, FIXTURE_TRANSCRIPT, FIXTURE_LANGUAGE, _settings=_default_settings()),
            timeout=30.0,
        )
...
async def test_does_not_block_event_loop(monkeypatch: pytest.MonkeyPatch):
    ...
    async def slow_path() -> None:
        await align(audio, FIXTURE_TRANSCRIPT, FIXTURE_LANGUAGE, _settings=_default_settings())
...
async def test_returns_word_objects():
    ...
    audio = _load_fixture()
    words = await align(audio, FIXTURE_TRANSCRIPT, FIXTURE_LANGUAGE, _settings=_default_settings())
...
async def test_second_call_is_faster(monkeypatch: pytest.MonkeyPatch):
    ...
    t0 = time.monotonic()
    await align(audio, FIXTURE_TRANSCRIPT, FIXTURE_LANGUAGE, _settings=s)
...
async def test_concurrent_calls_do_not_corrupt():
    ...
    results = await asyncio.gather(
        align(audio, FIXTURE_TRANSCRIPT, FIXTURE_LANGUAGE, _settings=s),
        align(audio, FIXTURE_TRANSCRIPT, FIXTURE_LANGUAGE, _settings=s),
    )
